...
Historically, the initial final Android attempt failed with `SDK location not found`. The coordinator subsequently verified that earlier build using `--no-daemon`, User SDK/JDK variables and the Windows certificate-root configuration (`BUILD SUCCESSFUL in 25s`, 186 tasks). On 2026-09-26 the configured and previously supplied SDK paths are unavailable again, while JDK21 exists. Current Android compilation evidence therefore comes from successful candidate CI, not that older local log. The browser matrix has now run; physical-device acceptance remains separate.
