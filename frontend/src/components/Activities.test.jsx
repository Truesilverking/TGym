...
it('explains a storage failure without exposing the native exception',async()=>{useStore.setState({flushPersistence:vi.fn().mockRejectedValue(Error('Native filesystem failure'))});await act(()=>root.render(<ActivityEditor close={close}/>));await click('Save');expect(host.querySelector('[role=alert]').textContent).toBe('Could not save. Check available storage and try again.');expect(host.textContent).not.toContain('Native filesystem failure')})
