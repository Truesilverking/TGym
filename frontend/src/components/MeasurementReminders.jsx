...
export default function MeasurementReminders({ initialMetric = 'weight', onRecord }) {
  ...
  return <><h3>{t('Measurement reminders')}</h3>
    ...
    <div className="list">{REMINDER_METRICS.map(([key, label]) => { const reminder = reminders.find(r => r.metric === key); const frequency=reminderInterval(reminderConfig(S,key)); return <Row key={key} title={t(label)} subtitle={reminder ? `${frequency.value} ${t({days:'Days',weeks:'Weeks',months:'Months'}[frequency.unit])} · ${t('Next: {0}',fmtDate(reminder.due, true))}` : t('Off')} accessory="chevron" onClick={() => setMetric(key)} /> })}</div>
    ...
    {metric === 'bodySize' && <p className="small muted">{t('Height')}</p>}
    {measurementDates(S, metric).at(-1) && <p className="small muted">{t('Last recorded')} · {fmtDate(measurementDates(S, metric).at(-1), true)}</p>}
