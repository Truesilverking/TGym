...
vi.mock('./ui.jsx',()=>({SelectRow:({options,onChange})=><div>{options.map(o=><button key={o.value} onClick={()=>onChange(o.value)}>{o.label}</button>)}</div>}))
...
it('shows corrected average, sample count and median and reuses period controls',async()=>{
  ...
  try {
    ...
    await act(()=>[...container.querySelectorAll('button')].find(b=>b.textContent==='All time').click())
    ...
    expect(container.textContent).toContain('Median1h 18m')
