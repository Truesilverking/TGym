...
import { backupToGoogleDrive, configuredGoogleClientId, forgetGoogleDriveToken, nativeDriveAuthAvailable, restoreFromGoogleDrive, synchronizeWithGoogleDrive } from '../lib/cloud-sync.js'
...
export default function RestoreSheet({ authorize = action => action() }) {
  ...
  const saveMerged = async next => {
    const saved = await backupToGoogleDrive(next, { interactive: false })
    replaceState(next, false)
    update(s => { s.cloudSync = { ...(s.cloudSync || {}), on: true, authorizedOnce: true, lastBackupAt: saved.at, lastAttemptAt: saved.at, lastFileId: saved.fileId, needsAuth: false, lastError: null } }, false)
  ...
  const backup = () => run(async () => {
    ...
    update(s => { s.cloudSync = { ...(s.cloudSync || {}), on: true, authorizedOnce: true, lastBackupAt: result.at, lastFileId: result.fileId, needsAuth: false, lastError: null } }, false)
    toast(t('Google Drive backup saved'))
  })
  const restore = () => run(async () => {
    const result = await restoreFromGoogleDrive(S, { interactive: true })
  ...
  return <><h3>{t('Restore')}</h3><div className="small muted" style={{ marginBottom: 8 }}>{t('Cloud backup status')}</div>
    <div className="list">
      ...
      <Row icon="history" iconTint="var(--blue)" title={t('Weekly Google Drive backup')}><Switch disabled={busy} checked={!!S.cloudSync?.on} onChange={v => { if (!v) forgetGoogleDriveToken(); update(s => { s.cloudSync = { ...(s.cloudSync || {}), on: v, needsAuth: v ? s.cloudSync?.needsAuth : false } }) }} /></Row>
