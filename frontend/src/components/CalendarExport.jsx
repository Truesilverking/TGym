...
async function capture(S, anchor, period) {
  ...
  try {
    flushSync(() => root.render(<><h2>TGym · {t('Training calendar')}</h2><ZoomCalendar S={S} initialAnchor={anchor} initialLevel={period === 'year' ? 'months' : period} exporting /></>))
...
export async function buildCalendarExport(S, anchor, period, format) {
  ...
  if (format === 'png') {
    ...
    const { png } = await capture(S, anchor, period)
  ...
  const section = async (date, p) => {
    ...
    const shot = await capture(S, date, p)
    ...
    } else for (const day of data.days.filter(d => p === 'week' || d.status !== 'rest')) {
