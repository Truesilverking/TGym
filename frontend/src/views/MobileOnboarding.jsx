...
export default function MobileOnboarding() {
  ...
  return <div className="narrow onboarding-panel">
    <div className="row between" style={{ marginBottom: 20 }}><div><h1>TGym</h1><div className="sub">{t('Initial setup')} · {step + 1}/7</div></div><Icon name="figureStrength" style={{ color: 'var(--acc)', fontSize: 34 }} /></div>
    <progress className="onboarding-progress" value={step + 1} max={7} aria-label={t('Initial setup')} />
    ...
    {step === 2 && <><h2>{t('Available equipment')}</h2><div className="muted small" style={{ marginBottom: 12 }}>{t('This only improves suggestions. You can always see every exercise.')}</div><div className="chips choice-grid">{EQUIPMENT.map(x => <button key={x} aria-pressed={equipment.has(x)} className={'chip' + (equipment.has(x) ? ' on' : '')} onClick={() => setEquipment(old => { const n = new Set(old); n.has(x) ? n.delete(x) : n.add(x); return n })}>{t(x)}</button>)}</div><Nav next={saveEquipment} /></>}
