...
const source=readFileSync(new URL('../../public/sw.js',import.meta.url),'utf8').replaceAll('__TGYM_CACHE__','tgym-app-1.15.25-test')
function worker({offline=false,failInstall=false}={}) {const handlers={}, data=new Map([['index.html',{body:'offline shell'}]]), caches={keys:async()=>['tgym-1.15.18-old','tgym-app-1.15.24-old','tgym-app-1.15.25-test','tgym-user-data','unrelated'],delete:vi.fn(),open:async()=>({addAll:()=>failInstall?Promise.reject(Error('offline')):Promise.resolve(),match:async key=>data.get(key),put:vi.fn()})};const self={location:{origin:'https://app.test'},addEventListener:(name,fn)=>handlers[name]=fn,clients:{claim:vi.fn()},skipWaiting:vi.fn()};vm.runInNewContext(source,{self,caches,URL,Response,fetch:()=>offline?Promise.reject(Error('offline')):Promise.resolve(new Response('ok'))});return {handlers,caches,self}}
