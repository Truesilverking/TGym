...
export const REPORT_COLORS = { completed: '#2878d0', missed: '#c93e4e', pending: '#747b87', rest: '#edf0f4' }
...
function grid(S, date, period, x, y, width, t, now) {
  ...
  data.days.forEach((day, i) => {
    ...
    svg += text(cx + cell / 2, cy + cell / 2 + (width < 400 ? 5 : 7), Number(day.iso.slice(-2)), width < 400 ? 15 : 21, 600, 'middle', day.status === 'rest' ? '#576172' : '#ffffff') + '</g>'
  })
  return svg + '</g>'
}
...
function page(S, anchor, period, months, overview, t, now, pageNumber, pageCount) {
  ...
  const height = period === 'week' ? 650 : overview ? 1700 : 1360
  ...
  svg += text(48, 181, `${summary.start} - ${summary.end}`, 20, 400, 'start', '#576172')
  ...
  ;[['completed', 'Completed'], ['missed', 'Not completed'], ['pending', 'Pending']].forEach(([status, label], i) => {
    const x = 48 + i * 306
    svg += rect(x, legendY - 18, 18, 18, REPORT_COLORS[status], 3) + text(x + 29, legendY - 2, t(label), 18)
  })
