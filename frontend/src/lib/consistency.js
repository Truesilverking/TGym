...
export function consistencyStats(S, start, end, now = new Date()) {
  ...
  for (const day of days) {
    ...
    if (day.status !== 'rest') result[day.status]++
...
export function nextScheduledWorkout(S, now = new Date()) {
  ...
  const dates = [today, S.scheduleStarted, ...Object.keys(state.dayPlan), ...workouts.map(w => w.d)].filter(d => /^\d{4}-\d{2}-\d{2}$/.test(d || '')).sort()
  ...
  for (const d = new Date(today + 'T12:00:00'); d <= end; d.setDate(d.getDate() + 1)) {
    const iso = isoOf(d)
    if (S.scheduleStarted && iso < S.scheduleStarted) continue
