...
import { releaseMessages } from './release-message.mjs'
...
for (const message of releaseMessages(version)) {
const body={validate_only:validate,message}
...
console.log(validate ? `FCM ${message.topic} credentials and message validated (no notification sent)` : `FCM accepted ${message.topic} update notification: ${result.name}`)
}
